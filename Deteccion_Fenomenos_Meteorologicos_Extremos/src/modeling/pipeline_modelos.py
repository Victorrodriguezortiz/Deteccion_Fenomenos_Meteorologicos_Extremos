...
class ExtremeEventModelPipeline:
    def __init__(
        self,
        X_train,
        y_train,
    ...
    def _create_models(self):
        ...
        if IMBLEARN_AVAILABLE:
            ...
            models["HistGradientBoosting Under"] = Pipeline([
                ("under", RandomUnderSampler(
                    sampling_strategy=0.20,
                    random_state=self.random_state
                )),
            ...
            models["Logistic Regression Over"] = Pipeline([
            ...
            models["Random Forest Over"] = Pipeline([
                ("over", RandomOverSampler(
                    sampling_strategy=0.20,
                    random_state=self.random_state
                )),
            ...
            models["Extra Trees Over"] = Pipeline([
                ("over", RandomOverSampler(
                    sampling_strategy=0.20,
                    random_state=self.random_state
                )),
            ...
            models["HistGradientBoosting Over"] = Pipeline([
                ("over", RandomOverSampler(
                    sampling_strategy=0.20,
                    random_state=self.random_state
                )),
        ...
        for model_name in model_names:
            ...
            try:
                ...
                # -------------------------------
                    ...
                    "HistGradientBoosting Under",
                    "HistGradientBoosting Over"
                ]:
                    pipeline = self.pipelines[model_name]
    ...
    def plot_shap_bar(self, model_name, max_display=20):
        if model_name not in self.shap_values:
            print(f"⚠️ No hay SHAP calculado para {model_name}")
            return
    ...
    def get_feature_importance_df(self, model_name, top_n=30):
        ...
        if hasattr(model, "feature_importances_"):
            ...
            valores = np.abs(model.coef_).ravel()

        else:
            print(f"⚠️ {model_name} no tiene feature_importances_ ni coef_.")
            return None
